...
namespace ZDragon.Transpilers.Components {
    public class ComponentTranspiler_old {
        ...
        private void TranspileSystemNode(SystemNode node) {
            ...
            if (contains.Count() == 0) {
                ...
                foreach (var c in contains) {
                    ...
                    var _deprecated = bool.Parse(container.GetAttribute("Deprecated", "false").ToLower());
                    var _changed = bool.Parse(container.GetAttribute("Change", "false").ToLower());
                    
                    var _tags = "";
                    if (_deprecated) {
                        _tags = $", $tags=\"deprecated\"";
                    }
                    else if (!_deprecated && _changed) {
                        _tags = $", $tags=\"change\"";
                    }
                    ...
                    systemParts.Add($@"    {componentType}({container.Id}, ""{_name}"", ""v{_version},{_tech}"", ""{_description}""{_tags})");
                    ...
                    foreach (var interaction in interactions) {
                        ...
                        relations.Add($"    Rel({container.Id}, {interactionParts[0]}, {interactionParts[1]}, {interactionParts[2]})");
                    }
                }
        ...
        public ComponentTranspiler_old(CompilationResult compilationresult) {
